...
import { Bell, ChevronDown, CircleHelp, LayoutDashboard, LineChart, LogOut, Menu, Settings, Star, WalletCards } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-[#f0ede7] text-[#17191a]">
      <header className="sticky top-0 z-50 border-b border-[#dedbd5] bg-white/95 backdrop-blur">
        <div className="mx-auto flex h-[70px] max-w-[1400px] items-center gap-8 px-4 sm:px-6 lg:px-8">
          <AxiWordmark />
          <nav className="hidden flex-1 items-center gap-1 lg:flex">
            {primary.map(({ label, href, icon: Icon }) => (
              <Link key={label} href={href} className={`flex items-center gap-2 rounded-md px-4 py-2.5 text-xs font-bold transition ${active === label ? "bg-[#f6f4f0] text-[#d61f2c]" : "text-[#66696a] hover:bg-[#f6f4f0] hover:text-[#17191a]"}`}>
                <Icon className="h-4 w-4" />{label}
              </Link>
            ))}
            <Link href="/deposit/" className={`ml-2 flex items-center gap-2 rounded-md px-4 py-2.5 text-xs font-bold ${active === "Funding" ? "bg-[#f6f4f0] text-[#d61f2c]" : "text-[#66696a] hover:bg-[#f6f4f0]"}`}><WalletCards className="h-4 w-4" />Funding</Link>
          </nav>
          <div className="ml-auto flex items-center gap-2">
            ...
            <Link href="/markets/" className="rounded-md p-2.5 text-[#17191a] lg:hidden" aria-label="Open app navigation"><Menu className="h-5 w-5" /></Link>
          </div>
        </div>
      </header>
      <main>{children}</main>
