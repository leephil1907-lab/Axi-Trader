    ...
    <div className="min-h-screen bg-white flex flex-col">
      <header className="sticky top-0 z-50 bg-white border-b border-[#D9D3CB] px-4 py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3"><button onClick={() => selectedTx ? setSelectedTx(null) : window.history.back()} className="p-2 -ml-2 rounded-lg hover:bg-[#F5F2ED]"><ArrowLeft size={20} /></button><h1 className="text-lg font-bold">{selectedTx ? "Transaction Details" : "Wallet"}</h1></div>
          {!selectedTx && <button onClick={() => void load()} className="p-2 rounded-lg hover:bg-[#F5F2ED]" aria-label="Refresh wallet"><RefreshCw size={17} /></button>}
        </div>
      </header>
        ...
        {loading ? <div className="p-8 text-center text-sm text-[#6B6560]">Loading account records…</div> : !selectedTx ? (
          <>
            <div className="p-6 bg-[#1A1A1A] rounded-2xl mb-6">
              <div className="flex items-center justify-between mb-2"><p className="text-xs text-white/40 font-bold uppercase tracking-wider">Available Balance</p><button onClick={() => setHideBalance(!hideBalance)} className="text-white/50">{hideBalance ? <EyeOff size={15} /> : <Eye size={15} />}</button></div>
              <p className="text-3xl font-black text-white">{hideBalance ? "••••••" : formatMoney(Number(portfolio?.user.balance || 0), currency)}</p>
              <p className="mt-2 text-xs text-white/40">Source: authenticated PostgreSQL account record</p>
            </div>
            ...
            {filtered.length === 0 ? <div className="p-6 rounded-2xl bg-[#F5F2ED] text-sm text-[#6B6560]">No account transactions have been recorded.</div> : <div className="space-y-2">{filtered.map(t => <motion.button key={t.id} whileTap={{scale:.98}} onClick={() => setSelectedTx(t.id)} className="w-full p-4 bg-[#F5F2ED] rounded-xl text-left flex items-center gap-3"><div className={`w-10 h-10 rounded-full flex items-center justify-center ${t.type === "deposit" ? "bg-[#22A958]/10" : "bg-[#D31C2B]/10"}`}>{t.type === "deposit" ? <ArrowUpRight size={18} className="text-[#22A958]"/> : <ArrowDownRight size={18} className="text-[#D31C2B]"/>}</div><div className="flex-1"><p className="text-sm font-bold capitalize">{t.type}</p><p className="text-[10px] text-[#9B9590]">{t.method} · {new Date(t.createdAt).toLocaleString()}</p></div><div className="text-right"><p className={`text-sm font-bold ${t.type === "deposit" ? "text-[#22A958]" : "text-[#D31C2B]"}`}>{t.type === "deposit" ? "+" : "-"}{formatMoney(Number(t.amount), t.currency)}</p><span className="text-[9px] px-1.5 py-0.5 rounded font-bold uppercase bg-white">{t.status}</span></div></motion.button>)}</div>}
          </>
        ) : tx ? (
          <div><div className="p-6 bg-[#1A1A1A] rounded-2xl mb-6 text-center"><p className={`text-3xl font-black ${tx.type === "deposit" ? "text-[#22A958]" : "text-[#D31C2B]"}`}>{tx.type === "deposit" ? "+" : "-"}{formatMoney(Number(tx.amount), tx.currency)}</p><p className="text-sm text-white/50 mt-1 capitalize">{tx.type}</p><span className="inline-block mt-3 px-3 py-1 rounded-lg text-xs font-bold uppercase bg-white/10 text-white">{tx.status}</span></div><div className="p-4 bg-[#F5F2ED] rounded-2xl space-y-3"><div className="flex justify-between text-sm"><span className="text-[#9B9590]">Transaction ID</span><span className="font-mono font-bold">{tx.id}</span></div><div className="flex justify-between text-sm"><span className="text-[#9B9590]">Method</span><span className="font-bold">{tx.method}</span></div><div className="flex justify-between text-sm"><span className="text-[#9B9590]">Currency</span><span className="font-bold">{tx.currency}</span></div><div className="flex justify-between text-sm"><span className="text-[#9B9590]">Date</span><span className="font-bold">{new Date(tx.createdAt).toLocaleString()}</span></div></div></div>
        ) : <div className="p-6 text-sm text-[#6B6560]">Transaction not found.</div>}
      </div><LiveChatBot />
    </div>
  );
}
