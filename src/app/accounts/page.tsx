  ...
  return <AxiAppShell active="Accounts">
    ...
    <div className="mx-auto max-w-[1400px] px-4 py-8 sm:px-6 lg:px-8">
      <section className="grid gap-5 lg:grid-cols-[1.4fr_.6fr]">
        ...
        <div className="rounded-md border border-[#dedbd5] bg-white p-6"><div className="flex items-center gap-3"><WalletCards className="h-5 w-5 text-[#d61f2c]" /><h2 className="font-black">Portfolio</h2></div><div className="mt-6 grid grid-cols-2 gap-4"><div><span className="text-[9px] font-bold uppercase text-[#8a8d8e]">Balance</span><strong className="mt-1 block text-lg">{user?.balance == null ? "—" : `${currency} ${user.balance.toLocaleString()}`}</strong></div><div><span className="text-[9px] font-bold uppercase text-[#8a8d8e]">Equity</span><strong className="mt-1 block text-lg">{user?.equity == null ? "—" : `${currency} ${user.equity.toLocaleString()}`}</strong></div></div><Link href="/dashboard/" className="mt-6 flex items-center justify-center gap-2 rounded-md bg-[#d61f2c] py-3 text-xs font-extrabold uppercase tracking-wider text-white">Open dashboard <ArrowRight className="h-4 w-4" /></Link></div>
      </section>

      <div className="mt-8 flex items-end justify-between gap-4"><div><p className="text-[10px] font-extrabold uppercase tracking-[.18em] text-[#d61f2c]">Trading terminals</p><h2 className="mt-1 text-2xl font-black">Choose your platform</h2></div><Link href="/helpcenter/" className="hidden text-xs font-bold text-[#66696a] sm:block">Platform help <ExternalLink className="ml-1 inline h-3.5 w-3.5" /></Link></div>
