...
export default function TradingPage() {
  ...
  return <AxiAppShell active="Markets">
    ...
    <div className="mx-auto max-w-[1500px] px-4 py-6 sm:px-6 lg:px-8"><div className="grid gap-5 xl:grid-cols-[260px_minmax(0,1fr)_360px]">
      ...
      <aside className="order-3 space-y-4"><section className="rounded-md border border-[#dedbd5] bg-white p-5"><div className="flex items-center gap-3"><div className="grid h-9 w-9 place-items-center rounded-md bg-[#17191a] text-white"><BarChart3 className="h-4 w-4" /></div><div><p className="text-[10px] font-extrabold uppercase tracking-wider text-[#777a7b]">Order ticket</p><h2 className="text-lg font-black">{symbol || "Select market"}</h2></div></div><div className="mt-5 grid grid-cols-2 gap-1 rounded-md bg-[#f0ede7] p-1"><button type="button" onClick={() => setSide("buy")} className={`rounded py-3 text-xs font-extrabold uppercase tracking-wider ${side === "buy" ? "bg-white text-[#d61f2c] shadow-sm" : "text-[#777a7b]"}`}>Buy</button><button type="button" onClick={() => setSide("sell")} className={`rounded py-3 text-xs font-extrabold uppercase tracking-wider ${side === "sell" ? "bg-white text-[#d61f2c] shadow-sm" : "text-[#777a7b]"}`}>Sell</button></div><label className="mt-5 block text-[10px] font-extrabold uppercase tracking-wider text-[#777a7b]">Trade size<input value={volume} onChange={(e) => setVolume(e.target.value)} type="number" min="0" step="any" placeholder="Enter size" className="mt-2 h-11 w-full rounded-md border border-[#cfd0ce] px-3 text-sm outline-none focus:border-[#d61f2c]" /></label><div className="mt-4 grid grid-cols-2 gap-3"><label className="text-[10px] font-extrabold uppercase tracking-wider text-[#777a7b]">Stop loss<input value={stopLoss} onChange={(e) => setStopLoss(e.target.value)} type="number" step="any" placeholder="Optional" className="mt-2 h-10 w-full rounded-md border border-[#cfd0ce] px-3 text-sm outline-none focus:border-[#d61f2c]" /></label><label className="text-[10px] font-extrabold uppercase tracking-wider text-[#777a7b]">Take profit<input value={takeProfit} onChange={(e) => setTakeProfit(e.target.value)} type="number" step="any" placeholder="Optional" className="mt-2 h-10 w-full rounded-md border border-[#cfd0ce] px-3 text-sm outline-none focus:border-[#d61f2c]" /></label></div><div className="mt-5 grid grid-cols-2 gap-3 rounded-md bg-[#f6f4f0] p-3 text-xs"><div><span className="text-[9px] font-bold uppercase text-[#8a8d8e]">Side</span><strong className="mt-1 block uppercase">{side}</strong></div><div><span className="text-[9px] font-bold uppercase text-[#8a8d8e]">Size</span><strong className="mt-1 block">{volume || "—"}</strong></div></div>{message && <div role="alert" className="mt-4 rounded-md border border-[#efc5c8] bg-[#fff5f5] p-3 text-xs leading-5 text-[#9f1722]">{message}</div>}<button type="button" disabled={busy || !symbol || !live} onClick={submit} className="mt-5 flex h-12 w-full items-center justify-center gap-2 rounded-md bg-[#17191a] text-xs font-extrabold uppercase tracking-wider text-white hover:bg-[#2a2c2d] disabled:cursor-not-allowed disabled:opacity-45">{busy ? "Sending order…" : `Place ${side === "buy" ? "Buy" : "Sell"} order`}<ArrowRight className="h-4 w-4" /></button></section><section className="rounded-md bg-[#17191a] p-5 text-white"><div className="flex items-center gap-3"><Gauge className="text-[#f5c842]" /><h2 className="font-black">Risk & execution</h2></div><p className="mt-3 text-xs leading-5 text-white/55">Market orders are submitted only when a live provider quote is available. Execution is confirmed only by the connected broker gateway.</p></section><section className="rounded-md border border-[#dedbd5] bg-white p-5"><div className="flex items-center gap-3"><ShieldCheck className="text-[#d61f2c]" /><h2 className="font-black">TradingView analysis</h2></div><p className="mt-3 text-xs leading-5 text-[#777a7b]">TradingView Advanced Charts are integrated into the current Axi Trading Platform experience. Broker execution and account pricing remain separate services.</p><div className="mt-3 flex items-start gap-2 text-[10px] leading-4 text-[#8a8d8e]"><AlertTriangle className="mt-0.5 h-3.5 w-3.5 shrink-0" />Charts are for analysis; always verify the executable quote shown by the connected broker.</div></section></aside>
    </div></div>
  </AxiAppShell>;
}
