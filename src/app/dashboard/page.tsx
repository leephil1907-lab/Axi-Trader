...
export default function DashboardPage() {
  ...
  if (loading) return <div className="min-h-screen grid place-items-center bg-[#f0ede7]"><div className="h-9 w-9 animate-spin rounded-full border-2 border-[#e4002e] border-t-transparent" /></div>;
  if (!data) return <div className="min-h-screen grid place-items-center bg-[#f0ede7] p-6"><div className="max-w-md rounded-lg border border-[#dedbd5] bg-white p-7 text-center"><h1 className="text-xl font-black">Account unavailable</h1><p className="mt-2 text-sm text-[#6c6f70]">{error || "We could not load your account."}</p><Link href="/login/" className="mt-5 inline-flex rounded-md bg-[#e4002e] px-5 py-3 text-xs font-extrabold uppercase tracking-wider text-white">Sign in</Link></div></div>;
  ...
  return <AxiAppShell active="Dashboard">
    <section className="bg-[#e4002e] text-white">
    ...
    <div className="mx-auto max-w-[1400px] px-4 py-7 sm:px-6 lg:px-8 lg:py-9">
      ...
      <div className="mt-7 grid gap-7 lg:grid-cols-[1fr_340px]">
        <section className="rounded-md border border-[#dedbd5] bg-white">
          <div className="flex items-center justify-between border-b border-[#e5e3df] px-5 py-4"><div><h2 className="font-black">My positions</h2><p className="mt-1 text-[11px] text-[#777a7b]">Open positions from your trading account</p></div><Link href="/positions/" className="text-[10px] font-extrabold uppercase tracking-wider text-[#e4002e]">See all</Link></div>
        ...
        <aside className="space-y-4">
          <div className="rounded-md border border-[#dedbd5] bg-white p-5"><div className="flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-[#e4002e]" /><h2 className="font-black">Account status</h2></div><div className="mt-4 flex items-center justify-between border-t border-[#e5e3df] pt-4"><span className="text-xs text-[#777a7b]">Identity verification</span><span className="rounded-full bg-[#f0f4ef] px-2.5 py-1 text-[10px] font-bold uppercase text-[#32734d]">{user.kycStatus || "Pending"}</span></div>{user.kycStatus !== "verified" && <Link href="/verify/" className="mt-4 block rounded-md bg-[#e4002e] px-4 py-3 text-center text-[10px] font-extrabold uppercase tracking-wider text-white">Verify identity</Link>}<Link href="/settings/" className="mt-4 block text-xs font-bold text-[#e4002e]">Manage account →</Link></div>
      ...
      <section className="mt-7 rounded-md border border-[#dedbd5] bg-white"><div className="flex items-center justify-between border-b border-[#e5e3df] px-5 py-4"><div><h2 className="font-black">Recent activity</h2><p className="mt-1 text-[11px] text-[#777a7b]">Funding and account transactions</p></div><Link href="/deposit/" className="text-[10px] font-extrabold uppercase tracking-wider text-[#e4002e]">Funding</Link></div>{transactions.length === 0 ? <div className="p-8 text-center text-sm text-[#777a7b]">No account transactions yet.</div> : transactions.slice(0, 6).map((transaction: any) => <div key={transaction.id} className="flex items-center justify-between gap-4 border-t border-[#e5e3df] px-5 py-4"><div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-full bg-[#f6f4f0]"><ArrowDownToLine className="h-4 w-4 text-[#e4002e]" /></span><div><p className="text-xs font-bold">{transaction.type}</p><p className="text-[10px] text-[#777a7b]">{transaction.method || "Account transaction"}</p></div></div><div className="text-right"><p className="text-xs font-bold">{formatMoney(Number(transaction.amount || 0), transaction.currency || currency)}</p><p className="text-[10px] uppercase text-[#777a7b]">{transaction.status}</p></div></div>)}</section>
    </div>
