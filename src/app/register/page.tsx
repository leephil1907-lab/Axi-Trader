    ...
    <main className="min-h-screen bg-[#f0ede7] text-[#17191a]">
      <header className="border-b border-[#dedbd5] bg-white">
        <div className="container-axi flex h-[72px] items-center justify-between">
          <AxiLogo />
          <p className="text-xs font-semibold text-[#6c6f70]">Already registered? <Link href="/login/" className="font-bold text-[#d61f2c] hover:underline">Sign in</Link></p>
        </div>
      </header>
        ...
        <div className="mx-auto max-w-[900px]">
          <div className="mb-7 rounded-[5px] border border-[#dedbd5] bg-white px-5 py-5 sm:px-8">
            <div className="mb-5 flex items-center justify-between gap-3">
              <div>
                <p className="text-[10px] font-extrabold uppercase tracking-[0.18em] text-[#d61f2c]">Open an account</p>
            ...
            <div className="h-1.5 overflow-hidden rounded-full bg-[#e7e4df]" aria-label={`Step ${step} of ${TOTAL_STEPS}`}>
              <div className="h-full rounded-full bg-[#d61f2c] transition-all duration-300" style={{ width: progress }} />
            </div>
            ...
            <section className="rounded-[5px] border border-[#dedbd5] bg-white p-6 sm:p-9">
              ...
              {step === 1 && (
                <div>
                  ...
                  <div className="relative">
                    ...
                    <select id="country" value={form.country} onChange={(event) => update("country", event.target.value)} className="h-12 w-full appearance-none rounded-[4px] border border-[#cfd0ce] bg-white pl-11 pr-4 text-sm outline-none focus:border-[#d61f2c] focus:ring-2 focus:ring-[#d61f2c]/10">
              ...
              {step === 3 && (
                <div>
                  ...
                  <div className="space-y-5">
                    <div className="grid gap-5 sm:grid-cols-2"><div><label htmlFor="firstName" className="mb-2 block text-xs font-bold">First name</label><div className="relative"><UserRound className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[#7b7e7f]" /><input id="firstName" autoComplete="given-name" value={form.firstName} onChange={(event) => update("firstName", event.target.value)} className="h-12 w-full rounded-[4px] border border-[#cfd0ce] pl-11 pr-4 text-sm outline-none focus:border-[#d61f2c] focus:ring-2 focus:ring-[#d61f2c]/10" placeholder="First name" /></div></div><div><label htmlFor="lastName" className="mb-2 block text-xs font-bold">Last name</label><input id="lastName" autoComplete="family-name" value={form.lastName} onChange={(event) => update("lastName", event.target.value)} className="h-12 w-full rounded-[4px] border border-[#cfd0ce] px-4 text-sm outline-none focus:border-[#d61f2c] focus:ring-2 focus:ring-[#d61f2c]/10" placeholder="Last name" /></div></div>
                    <div><label htmlFor="phone" className="mb-2 block text-xs font-bold">Mobile number</label><div className="relative"><Phone className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[#7b7e7f]" /><input id="phone" type="tel" autoComplete="tel" value={form.phone} onChange={(event) => update("phone", event.target.value)} className="h-12 w-full rounded-[4px] border border-[#cfd0ce] pl-11 pr-4 text-sm outline-none focus:border-[#d61f2c] focus:ring-2 focus:ring-[#d61f2c]/10" placeholder="Mobile number" /></div></div>
                  </div>
                </div>
              )}
              ...
              {step === 5 && (
                <form onSubmit={submit}>
                  ...
                  <div className="space-y-4"><label className="flex gap-3 text-sm leading-6"><input type="checkbox" checked={form.terms} onChange={(event) => update("terms", event.target.checked)} className="mt-1 h-4 w-4 shrink-0 accent-[#d61f2c]" /><span>I agree to the Terms and Conditions and acknowledge the Privacy Policy.</span></label><label className="flex gap-3 text-sm leading-6"><input type="checkbox" checked={form.marketing} onChange={(event) => update("marketing", event.target.checked)} className="mt-1 h-4 w-4 shrink-0 accent-[#d61f2c]" /><span>I would like to receive market analysis and promotional communications. <span className="text-[#777a7b]">Optional</span></span></label></div>
                  <button type="submit" disabled={loading} className="mt-7 flex h-12 w-full items-center justify-center gap-2 rounded-[4px] bg-[#d61f2c] px-5 text-xs font-extrabold uppercase tracking-[0.12em] text-white transition hover:bg-[#b91824] disabled:cursor-not-allowed disabled:opacity-60">{loading ? "Creating account" : "Create account"}{!loading && <ArrowRight className="h-4 w-4" />}</button>
                </form>
              )}
              ...
              <div className="mt-9 flex items-center justify-between gap-4 border-t border-[#e8e5e1] pt-6">
                ...
                {step < TOTAL_STEPS && <button type="button" onClick={next} className="inline-flex h-11 items-center gap-2 rounded-[4px] bg-[#d61f2c] px-5 text-xs font-extrabold uppercase tracking-[0.1em] text-white hover:bg-[#b91824]">Continue <ArrowRight className="h-4 w-4" /></button>}
              </div>
            </section>
